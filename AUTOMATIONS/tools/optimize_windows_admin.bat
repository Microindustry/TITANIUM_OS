...
echo   ^<Triggers^>
echo     ^<LogonTrigger^>^<Enabled^>true^</Enabled^>^<UserId^>%COMPUTERNAME%\%USERNAME%^</UserId^>^<Delay^>PT20S^</Delay^>^</LogonTrigger^>
echo   ^</Triggers^>
echo   ^<Principals^>
...
echo       ^<Arguments^>/c "%TI%\START_LOGIN.bat"^</Arguments^>
echo     ^</Exec^>
echo   ^</Actions^>
echo ^</Task^>
) > "%XMLFILE%"
