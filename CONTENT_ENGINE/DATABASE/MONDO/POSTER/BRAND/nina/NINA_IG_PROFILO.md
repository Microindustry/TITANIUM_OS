<!-- TOC -->

- [Instagram Nina  kit profilo (account SEPARATO da microindustry)](#instagram-nina-kit-profilo-account-separato-da-microindustry)
  - [Foto profilo](#foto-profilo)
  - [Handle (da creare  scegli tu  verifica disponibilità)](#handle-da-creare-scegli-tu-verifica-disponibilità)
  - [Tipo account](#tipo-account)
  - [Nome (grassetto, max 30)](#nome-grassetto-max-30)
  - [Categoria](#categoria)
  - [Biografia (max 150)](#biografia-max-150)
  - [Link in bio](#link-in-bio)
  - [Ritmo  scaletta (coda pronta in POSTER/_SOCIAL_QUEUE_NINA/)](#ritmo-scaletta-coda-pronta-in-postersocialqueuenina)
  - [Nota](#nota)

<!-- /TOC -->

# Instagram Nina — kit profilo (account SEPARATO da microindustry)
*v1.0 · 2026-07-19 · canon-safe · mondo di Nina (buonanotte che insegna)*

## Foto profilo
`POSTER/BRAND/nina/nina_profilo.jpg` (volto Nina dalla copertina PRE_03, ritagliato tondo-safe).

## Handle (da creare — scegli tu + verifica disponibilità)
1. **@ilmondodinina.ms**  ← consigliato (dice cos'è)
2. @mondodinina
3. @nina.avventura
4. @nina.impara

## Tipo account
Professionale → **Creator** (adatto a un personaggio/publisher).

## Nome (grassetto, max 30)
```
Il Mondo di Nina
```

## Categoria
**Education**  *(alt: Digital creator)*

## Biografia (max 150)
```
🌙 Favola della buonanotte che insegna come funziona il mondo.
Atomi & Bit, tutto vero · @microindustry.ms
Si impara con le mani ✨🔧
```
*(~126 caratteri. L'@mention è la porta gemella tappabile verso il sistema reale.)*

## Link in bio
Cross-link alla porta gemella: **@microindustry.ms**
*(dopo: sito/Linktree con le due porte — sistema + Nina)*

## Ritmo & scaletta (coda pronta in POSTER/_SOCIAL_QUEUE_NINA/)
Buonanotte: **mercoledì + domenica ore 21:00** (si legge la sera).
Preambolo in ordine, poi episodi:
| Data | Post |
|---|---|
| mer 22/07 21:00 | PRE_01 · La porta |
| dom 26/07 21:00 | PRE_02 · Come funziona |
| mer 29/07 21:00 | PRE_03 · I personaggi |
| dom 02/08 21:00 | PRE_04 · Tutto è vero |
| mer 05/08 21:00 | EP_N2_01 · La bambina che chiedeva perché |
| dom 09/08 21:00 | EP_N2_02 · Il soffio di troppo |
| mer 12/08 21:00 | EP_N2_03 · Mille volte uguale |

Poi continua con gli EP_N2 successivi allo stesso ritmo.

## Nota
Gestione IDENTICA a microindustry (profilo brandizzato + Business Suite se colleghi una Pagina FB, o app IG). Tono diverso: dolce, meraviglia, non il registro tecnico dei pari. Save-trigger «leggilo stasera ad alta voce» già in ogni descrizione.
